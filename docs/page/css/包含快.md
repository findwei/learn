...
mdn文档解释包含快：*https://developer.mozilla.org/zh-CN/docs/Web/CSS/Containing_block*

**包含块**英语全称为**containing block**，实际上平时你在书写 CSS 时，大多数情况下你是感受不到它的存在，但是这玩意儿是确确实实存在的，在 CSS 规范中也是明确书写了的：

*https://drafts.csswg.org/css2/#containing-block-details*

<img src="https://cdn.jsdelivr.net/gh/findwei/learnImages@main/css/2022-08-14-142459.png" alt="image-20220814222458695" style="zoom:50%;" />
...
<img src="https://cdn.jsdelivr.net/gh/findwei/learnImages@main/css/2022-08-14-143451.png" alt="image-20220814223451349" style="zoom: 33%;" />
...
1. 一种是根元素（HTML 元素）所在的包含块，被称之为初始包含块（**initial containing block**）。对于浏览器而言，初始包含块的的大小等于视口 viewport 的大小，基点在画布的原点（视口左上角）。它是作为元素绝对定位和固定定位的参照物。
2. 一种是对于非根元素，对于非根元素的包含块判定就有几种不同的情况了。大致可以分为如下几种：
   - 如果元素的 positiion 是 relative 、static 或 sticky ，那么包含块由离它最近的**块容器**（block container）的内容区域（content area）的边缘建立。
   - 如果 position 属性是 fixed，那么包含块由视口建立。
   - 如果元素使用了 absolute 定位，则包含块由它的最近的 position 的值不是 static （也就是值为fixed、absolute、relative 或 sticky）的祖先元素的内边距区的边缘组成。

前面两条实际上都还比较好理解，第三条往往是初学者容易比较忽视的，看一看下面例子

> **块容器：** 不是说一定是display:block才叫块容器 **只要具有块特征的就属于块容器** 比如说 inline-block, block 或 list-item 元素,也可能会建立格式化上下文 (比如说 table container, flex container, grid container, 或者是 the block container 自身)。
>
> **块容器的特性**
>
> - 盒子模型
>   - **块容器**会遵循CSS盒子模型的规则，包括边距（margin）、边框（border）、内边距（padding）和内容（content）。同时这也是**块的特征**
> - 布局特性：
>   - 块容器通常占据其父容器的整个宽度。
>   - 每个块容器都会在其前后产生一个换行。
> - 嵌套能力
...
<img src="https://cdn.jsdelivr.net/gh/findwei/learnImages@main/css/2022-08-14-153548.png" alt="image-20220814233548188" style="zoom: 33%;" />
...
<img src="https://cdn.jsdelivr.net/gh/findwei/learnImages@main/css/2022-08-14-154347.png" alt="image-20220814234347149" style="zoom:33%;" />
