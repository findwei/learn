[TOC]

# java语言基本结构

## 基本数据类型

1. 基本数据类型 （8个）

   -  整型 (byte short int long)

     用第一个 bit来记录符号  0正数 1负数 

     > byte (8bit) 1字节  (8 位的数字)
     >
     >  byte 字节型 ====> 8bit 
     >
     > 1字节 == 8bit 0 0000000 2的8次方 = 256 种组合
     >
     > 用第一个 bit来记录符号  0正数 1负数 	所以 byte 类型的的数值范围是 ` -(2的7次方)  到 2的7次方减一 ` （-128 ~ 127）（减一是是因为0不存在正负一说）
     
     
     
     > short (16bit) 2字节
     >
     > ` -(2的15次方)  到 2的15次方减一 ` （-32768 ~ 32767）
     
     > int (32bit) 4字节
     >
     > ` -(2的31次方)  到 2的31次方减一 ` （-2147483648 ~ 2147483648）
     
     > long (64 bit) 8字节
     >
     > ` -(2的63次方)  到 2的63次方减一 ` （-18,446,744,073,709,551,616 ~ 18,446,744,073,709,551,615）
     

   -  浮点（float double）

        整数行 没有负数

        > float （32bit） 4字节 (32 位浮点数)
        > 根据IEEE754标准，浮点数float，符号位占1bit，指数占8bit，有效位占23bit，所以范围表示为3.4E-38~3.4E38，比long的范围大。
        >
        > 0 00000000 00000000000000000000000
        >
        > 第一位还是用语 `符号位` 第二位到第九位是 `指数位` 后面的 `底数位`

        > double （64bit） 8字节
        >
        > 0 00000000000 00000000000000000000000...
        >
        > 第一位还是用语 `符号位` 第二位到第十二位是 `指数位` 后面的 `底数位`

        **注意**：long 表示的是32 的数字, 取值范围就是-2,147,483,648 到2,147,483,647， float 表示的是32 位浮点数, 因为有指数加成, 取值范围就是1.4E-45 到3.4E+38， 所以我们才会说, float 的取值范围会比long 还要大。

   -  字符 （char）

        > char （16bit） 2字节  unicode 编码

   -  布尔 （boolean）

        > boolean (8bit)  1字节 

2. 引用类型（没有个数 因为可以自定义 下面几个常用的）
    - 数组[]
    - 类class (抽象类abstract class)
    - 接口 interface
    - 枚举 enum
    - 注解 @interface
    
    ------
    
    

## 常量与变量

1. 常量

   定义 : **常量指的是在程序运行中 `不能再次改变` 的值**

   作用：

    - 固定的值 便于程序计算 （例如3.1415926...）
    -  用来代表一个含义 （例如：用up 代表上 down 代表下）

   什么样的值算常量？

   	-  固定不变的值 （可以认为所有基本类型的值 固定不变的 是常量）
   	-  特殊的常量 (String) （String是一个引用类型 `他的值` 比较特殊可以用作常量）
   	-  自己创建的空间，存储的一个值，让他固定起来，不可改变

2. 变量

   定义：**变量指的是程序 `执行中可以改变` 的**
   
   ​	变量是一个内存空间（小容器）
   
   ​	变量空间在创建（声明）的时候，必须指定数据类型 和 变量空间名称
   
   ​	变量空间里面只能存储一个内容（值 | 引用）
   
   ​	变量空间的内容可以改变
   
   命名规则：
   
   ​	字母（区分大小写）、数字（数值是**不允许开头**） 、符号（不建议用_或者$ 开头） 、中文（不推荐）
   
   命名规约：一切要为了增强程序的可读性
   
   ​	类名字：最好使用大驼峰
   
   ​	变量名：最好使用小驼峰
   
   变量名创建/声明：
   
   ​	 `数据类型 变量名称`

   
   ​	 `int a`

   
   ​	 `String b`

   
   注意事项：
   
   ​	变量是一个空间 可以只创建空间 里面不存值 变量空间创建后里面是没有默认内容的 但是空的空间变量不能拿来使用 会编译错误
   
   ​	byte x; //声明一个空间说明 空间变量的名字是x 要求空间变量里面存储的类型是byte整型
   
   ​	x=1; // 将一个1赋值的x的空间内进行存储
   
   上面这两部操作 对于计算机底层是怎么做的 看下图
   
   > x是什么?   变量空间   存在哪儿?----栈内存里
   >
   > 1是什么?   1是常量    存在哪儿?----常量缓冲区
   >
   > =是什么?  =是赋值    做了什么?
   
   

```java
   //1.硬盘上创建了一个文件    Test.java
   //2.文件中的内容是我们编写的源代码(跟计算机发送指令)
   	public class Test{
   		public static void main(String[] args){
   				byte x;
   				x = 1;
   			}
   	}
   //3.将Test.java源文件---->编译---->Test.class
   //			内容  我们看不懂  字节码 留给计算机识别
   //4.执行---->内存中执行
   //			将硬盘上的Test.class内容  加载  到内存里
   //5.我们写好的指令  执行内存的空间  赋值  变化。。。。看下图
   ```

   
   

![基础原理图](https://cdn.jsdelivr.net/gh/findwei/learnImages@main/java/源码编译执行内存空间问题.jpg)

**堆内存：里面存储的数据是引用加内容  将一个堆内存里面的数据赋值给变量是赋值的当前数据的引用，使用变量的的时候也是根据引用找到的当前数据的内存**
   ​	
   
   ------
   
   给变量赋值数字  **整数默认是int** **小数默认是double** 所以下面要加字母来告知
   
   float e = 3.4F; 

      从double转化到float会有损失
      1  3.4  常量值
      存储在常量缓冲区中的
      常量值的存储形式肯定不是十进制   是以二进制形式存储
      1 ---->  32bit(int)
      3.4 -->  64bit(double)

   byte a = 1; 

      1 ---> 32bit  
      00000000 00000000 00000000 00000001
      a ---> 8bit
      00000001
      = 赋值的时候做了一个自动的决定   自动类型转化

   	
   long d = 2147483648L; 

      -2147483648    2147483647是int类型的取值范围
      如果创建的常量值超过以上范围
      程序编译检测的时候会认为  数字太大
      如果我们真的需要那么大的数字  必须在后面加L告知

​		

| 字符          | 字符串                                     |
| ------------- | ------------------------------------------ |
| char          | String                                     |
| 基本类型      | 引用类型                                   |
| ‘a’（单引号） | "a"     "abc"      ""      null （双引号） |

## 类型转化问题

​	基本类型

​		整型 浮点 字符 布尔

​	引用类型

1. 同种数据类型之间是可以直接进行赋值操作

2. 数据类型不同的空间 之间发生转换

   ​	同种大的类型之间才能发生转换

   ​		基本类型 ---- 基本类型  可以直接转换（自动 强制）

   ​		引用类型 ---- 引用类型  可以直接转换 （自动 强制 -- 上转型 下转型）

   ​		基本类型 ---- 引用类型  不可以直接转换 （间接 -- 包装类/封装类） 

3. 保证大数据类型一致的前提下

   1. 小数据类型相同 （都是整型 或者 都是浮点）

      ​	大空间变量**可以**直接存储小空间的数据

      ​	小空间变量**不可以**直接存储大空间的数据 `（需要强制转换）` 如果在转换过程中数值范围超过边界 **可能会有损失数据**

      - &    不会短路 所有的值都会看
      - && 会短路 如果遇到false 就返回false 不会看后面的值了 所以**效率高点**

      ```java
      	byte a = 1; int b = a //自动转换
        int a = 2 byte b =(byte)a; //强制转换 把a强制转换成byte类型
      	float x = 3.4F;   double y = x;//自动直接转化
      	double x = 3.4; float y = (float)x;//强制转换
      	//如果发现强制转换之前的数值比较大 强制转化这件事情是可以 但是 里面值会改变
      	int a = 1000; byte b = (byte)a;//编译好用 执行后 b存放的值一定发生变化
      ```

   2. 小数据类型不同

      整型 --- 浮点型

      ​	两边类型比较精确程度，浮点型精确程度更高，可以直接存放整数，反正则需要强制转换，任何一个浮点型都可以存放一个整型

      - &    不会短路 所有的值都会看
      - && 会短路 如果遇到false 就返回false 不会看后面的值了 所以**效率高点**


         ```java
         int a = 1;  float b = a;//自动直接转化
         float a =1.0F;  int b = (int)a;//强制类型转换
         ```

      整型 --- 字符串

      ​	每一个字符都对应这一个Unicode码   a--97

      - &    不会短路 所有的值都会看
      - && 会短路 如果遇到false 就返回false 不会看后面的值了 所以**效率高点**

      ```java
      char x = 'a';  int y = x;//自动转化  y--97
      int x = 97; char y = (char)x;//强制的转化
      ```

   3. 布尔类型很特殊

      不能与其他基本类型之间发生转化

## 运算符号

   定义：运算符 用来指明对于操作数的运算方式 

   我们可以按照操作数的数目来进行分类 （单目（a++）、双目（a + b ）、三目（boolean?：））

   * 算术运行  `+ - * / % ++ --`

   ```java
   //这里主要是注意一下 a++ ++a
   int a = 1
      int b = a++ // ++在后面是先赋值（先从变量里面取出来到栈里面 a:1） 后自增 （运算a++  a=2）赋值是赋值栈里面的
      int c = ++a// ++在前面 先自增 后赋值
      
   ```

   ![变量赋值与运算解析](https://cdn.jsdelivr.net/gh/findwei/learnImages@main/java/blfzysjx.png)

      **补充：上面最后一个例子(++运算符解析_03)第2、3步 a=++a 写成了 a=a++**

   * 赋值运算 `=`  赋值符号  将`=`右边的内容(值 引用) 存入=左边的变量空间内 `+=    -=    *=    /=    %=`

      ```java
      int x = 1
      for(int i=1;i<=10;i++){
         x=x++;
      }
      // 输出 x = 1

      ```

      byte x = 1; //常量32bit  = 自动转化

      x+=2; // x+2 +自动类型提升 3 =自动转化

      x = (byte)(x+2); //**编译出错**  类型从int转化成byte可能有损失

      x变量空间的值取出  从常量区取过来2  **加法运算（如果类型有差异会发生自动类型提升）**  结果重新存回x变量空间内
      x空间-->1    8bit

      常量区（默认32bit）-->2   32bit  所以在赋值回去的时候需要强制转换成byte

   * 关系运算（比较） `>    >=    <    <=    !=    ==`  (对象 instanceof 类)

     1. 区分=  和 == 区别

        =赋值符号      将=后面的结果(值 引用)存入=左边的变量空间内, 
        ==比较符号   比较==前面和==后面的元素(值 引用)是否一致

     2. 比较运算符的最终结果是什么?

        boolean   true  false

   * 逻辑运算

      逻辑运算符前后连接的两个都是boolean的值

      逻辑运算最终的结果还是boolean

      `&` 逻辑与（并且） `|` 逻辑或（或者） ` ^` 逻辑异或 `!` 逻辑非（取反） `&&` 短路与 `||` 短路或

      `^` 异(不同的)--->前后两个结果不一致  就可以 `ture ^ false` 返回 true

      `&&` 一个一个的向后面看，如果一个为 `true` 就看第二个一直看到最后一个，全部为 `true` 就返回 `true` 如果这个中间有一个 `false` 就终止不往后面看并将返回 `false`

      `||` 一个一个的向后面看，如果一个为 `false` 就看第二个一直看到最后一个，全部为 `false` 就返回 `false` 如果这个中间有一个 `true` 就终止不往后面看并将返回 `true`

         &&短路与
         1.短路 什么情况下才会发生短路?  当前面的值结果为false的时候会发生短路
         2.短路 到底短路的是什么?    短路的是&&之后所有计算的过程
         3.如果发生了短路情况  性能比&稍微好一点
         4.&逻辑与 和 &&短路与  从执行的最终结果来看没有任何区别
         5.短路与不一定提高了性能  只有当前面为false的时候才会发生短路  才会提高性能
               
         逻辑运算符前后连接的应该是两个boolean结果
         &逻辑与   可以理解为中文的"和","并且"意思  &前后两个条件必须同时满足 最终才为true
         |  逻辑或   可以理解为中文的"或者"  | 前后两个条件只要有一个满足 最终就为true
         ^逻辑异或   前后两个表达式只要结果不相同 最终结果就为true(专治强迫生)
         ！逻辑非   可以立即为中文的"不"   将原来的结果取反  

   * 位(bit)运算 （1看做true 0看做false）

     ​		`&`按位与  `|`按位或   `^`按位异或   `~`按位取反  `<<`按位左位移   `>>`按位右位移    `>>>`按位右位移(无符号)

     **位运算 转化成二进制表示形式来比较 比较后又转换成十进制**

     # 进制转化问题

     十六进制  0123456789ABCDEF

     十进制      0123456789 

     八进制      01234567

     二进制      01

     **十进制转二进制**  十进制数除以2取余数 将余数倒叙排列 即为有效位

     例如：60 转成二进制  111100

     **二进制转十进制**  从右至左开始计算  每一个元素乘以2的位置次幂 最终求和

     例如：111100 转十进制 结果：60 `1*2^5 + 1*2^4 + 1*2^3 + 1*2^2 + 0*2^1 + 0*2^0 =  32+16+8+4+0+0 =60`

     **二进制转八进制**  将二进制 `三` 个bit合并为一个  计算成十进制表示   八进制表示形式以0开头

     例如： 111  100  转八进制  结果：074  `1*2^2 + 1*2^1 + 1*2^0=4+3+1=7` `1*2^2 + 0*2^1 + 0*2^0=4+0+0=4`

     **二进制转十六进制**  将二进制 `四` 个bit合并为一个  计算成十进制表示   十六进制表示形式以0x开头
     
     例如： 0011 1100  转十六进制  结果：0x3C  `0*2^3 + 0*2^2 + 1*2^1 + 1*2^0=0+0+2+1=3` `1*2^3 + 1*2^2 + 0*2^1 + 0*2^0=8+4+0+0=12=C`

   [二、八、十、十六进制转换](https://www.cnblogs.com/gaizai/p/4233780.html)

# 源码 反码 补码（二进制表示数字）

计算机中不管是**整数还是负数** 存储的形式**都是以补码形式**来存储

注意 反码是一种表示形式  取反是一个计算过程(每一个位置都取反)

反码（One's Complement）的规则是：
- 正数的反码与原码相同
- 负数的反码是对应正数的原码各位取反（0变1，1变0）


补码（Two's Complement）是计算机中最常用的表示方法，它的规则是：
- 正数的补码与原码相同
- 负数的补码是对应正数的原码各位取反（0变1，1变0），然后加1



   |        | 6                                   | -6                                                    |
   | ------ | ----------------------------------- | ----------------------------------------------------- |
   | 原码： | 00000000 00000000 00000000 00000110 | 10000000 00000000 00000000 00000110                   |
   | 反码： | 00000000 00000000 00000000 00000110 | 11111111 11111111 11111111 11111001 符号不动 其余取反 |
   | 补码： | 00000000 00000000 00000000 00000110 | 11111111 11111111 11111111 11111010 反码+1            |


# 笔试题 -- 运算符
1. &和&&的区别

   1. & 可以视为**逻辑运算**符号**也**可以视为**位运算**  &&只能当作**逻辑运算**符号使用
   2. 如果&符号是当作逻辑运算符号来看 有如下区别
      - &    不会短路 所有的值都会看
      - && 会短路 如果遇到false 就返回false 不会看后面的值了 所以**效率高点**

2. 最有效率的方式计算2*8的结果

   **利用左位移 其中要有2的n次冥数**

   例如： 

   `2*8  2<<3 ` 相当于2乘以2的3次幂 `2*2^3=16   2<<3=2*2^3`

   `2*5=10  5<<1 = 5*2^1 = 10`   相当于5乘以2的1次幂

   `2*6=2*2*3=12  3<<2 = 3*2^1 = 6<<1 = 6*2^1` 相当于3乘以2的2次幂 也可以写成 6乘以2的1次幂

3. 两个变量 int a= 1; int b = 2; 如何将两个变量的值进行交换

   1. 借助第三个变量 int c

      ```
      int c = a;
      	a = b;
      	b = c;
      ```

      

   2. 利用运算

      ```
      int a = 1;
      int b = 2;
      a = a + b  3 = 1 + 2 
      b = a - b  1 = 3 - 2
      a = a - b  2 = 3 - 1
      //这种的好处是省略了一个新的空间  不好在于 可能会在+产生值越界
      ```

     

   3. 利用异或（**一个数字异或同一个数字两次  值不会改变**）

      ```
      int a = 1;
      int b = 2;
      a = a ^ b  
      b = a ^ b  // b = a ^ b ^ b
      a = a ^ b  // a = a ^ a ^ b
      //这种的好处是省略了一个新的空间  不好在于 可能会在+产生值越界
      
      1 ^ 2 
      001
      010
      011 = 3
      3 ^ 2
      011
      010
      001 = 1
      3 ^ 1
      011
      001
      010 = 2
      
      ```
